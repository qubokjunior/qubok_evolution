...
assert(packageJson.version === "0.1.0-milestone.41", "package.json must expose 0.1.0-milestone.41.");
...
assert(appVersion.includes("PROJECT_MILESTONE_LABEL = \"m41\""), "appVersion must expose m41 label.");
assert(readme.includes("0.1.0-milestone.41"), "README.md must expose current milestone version.");
assert(readme.includes("Current status: m41"), "README.md must expose current status m41.");
for (const marker of ["| m32 |", "| m33 |", "| m34 |", "| m35 |", "| m36 |", "| m37 |", "| m38 |", "| m39 |", "| m40 |", "| m41 |"]) assert(milestones.includes(marker), "docs/milestones.md missing marker: " + marker);
