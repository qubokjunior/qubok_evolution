...
assert(packageJson.version === "0.1.0-milestone.31", "package.json version must be 0.1.0-milestone.31.");
assert(appVersion.includes('PROJECT_VERSION = "0.1.0-milestone.31"'), "appVersion must expose milestone.31.");
assert(appVersion.includes('PROJECT_MILESTONE_LABEL = "m31"'), "appVersion must expose m31 overlay label.");
...
assert(lifecyclePressureTest.includes("death -> reusable slot -> birth"), "lifecycle pressure test must document the m31 loop.");
...
assert(readme.includes("0.1.0-milestone.31"), "README must expose the current milestone version.");
assert(readme.includes("docs/milestones.md"), "README must link the milestone index.");
assert(milestones.includes("| m31 |"), "milestones index must include m31.");
