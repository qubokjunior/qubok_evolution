...
assert(packageJson.version === "0.1.0-milestone.47", "package.json version must be 0.1.0-milestone.47.");
assert(appVersion.includes('PROJECT_VERSION = "0.1.0-milestone.47"'), "appVersion must expose milestone.46.");
assert(appVersion.includes("PROJECT_MILESTONE = 47"), "appVersion must expose milestone number 45.");
assert(appVersion.includes('PROJECT_MILESTONE_LABEL = "m47"'), "appVersion must expose m42 overlay label.");

assert(readme.includes("m47 complete"), "README must expose current m47 complete status.");
assert(readme.includes("field-force separation"), "README must expose the current m46 shipped field track.");
...
assert(repoStatusTest.includes("0.1.0-milestone.47"), "repo status test must validate m41 status sync.");
