...
export const PERF_METRIC_NAMES = [
  ...
  "sensorSectorWrites",
  "sensorFoodVisibleCount",
  "sensorFoodSectorWrites",
  "sensorObstacleSectorWrites",
  "sensorObstacleMaskCellChecks",
  ...
  "avgVisibleNeighborsPerAgent",
  "predatorPreyMs",
  "attacksThisStep",
  "killsThisStep",
  "predatorDamageDealt",
  "predatorEnergyGained",
  "reproductionMs",
  "birthsThisStep",
  "reproductionEligibleCount",
  "blockedBirthsByCapacity",
  "reusableSlotCount",
...
export type PerfMetricStats = {
  readonly name: PerfMetricName;
...
export type PerfMetricsSnapshot = {
  readonly version: typeof PERF_METRICS_VERSION;
  readonly sampleSerial: number;
  readonly metrics: Record<PerfMetricName, PerfMetricStats>;
  readonly values: Record<PerfMetricName, number>;
};

export type PerfMetricsBus = {
  readonly record: (name: PerfMetricName, value: number) => void;
  readonly add: (name: PerfMetricName, delta: number) => void;
  readonly getLatest: (name: PerfMetricName) => number;
  readonly getStats: (name: PerfMetricName) => PerfMetricStats;
  readonly beginScope: (name: PerfMetricName) => () => number;
  readonly makeSnapshot: () => PerfMetricsSnapshot;
  readonly reset: () => void;
};

export type PerfMetricsBusOptions = {
  readonly now?: () => number;
};

type MutablePerfMetricStats = {
  name: PerfMetricName;
  latest: number;
  average: number;
  min: number;
...
export function createPerfMetricsBus(options: PerfMetricsBusOptions = {}): PerfMetricsBus {
  ...
  const reset = (): void => {
    ...
    for (const name of PERF_METRIC_NAMES) {
      statsByName.set(name, createEmptyStats(name));
    }

    sampleSerial = 0;
  };

  const getMutableStats = (name: PerfMetricName): MutablePerfMetricStats => {
  ...
  const record = (name: PerfMetricName, value: number): void => {
    if (!Number.isFinite(value)) {
      throw new Error(`Metric ${name} must be finite. Received: ${value}`);
    }
    ...
    stats.min = stats.count === 1 ? value : Math.min(stats.min, value);
    stats.max = stats.count === 1 ? value : Math.max(stats.max, value);
    sampleSerial += 1;
  };

  const add = (name: PerfMetricName, delta: number): void => {
    const current = getMutableStats(name).latest;
    record(name, current + delta);
  };
  ...
  const beginScope = (name: PerfMetricName): (() => number) => {
    const start = now();
  ...
  const makeSnapshot = (): PerfMetricsSnapshot => {
    ...
    for (const name of PERF_METRIC_NAMES) {
      const stats = freezeStats(getMutableStats(name));
      metrics[name] = stats;
      values[name] = stats.latest;
    }

    return {
      version: PERF_METRICS_VERSION,
      sampleSerial,
      metrics,
      values
    };
...
function createEmptyStats(name: PerfMetricName): MutablePerfMetricStats {
  return {
    name,
    latest: 0,
    average: 0,
...
function freezeStats(stats: MutablePerfMetricStats): PerfMetricStats {
  return {
    name: stats.name,
    latest: stats.latest,
