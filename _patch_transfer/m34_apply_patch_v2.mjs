...
if (branch !== "m34-terrain-debug-render-layer") throw new Error("Expected m34-terrain-debug-render-layer, got " + branch);

patchPixiRenderer();
patchDebugOverlay();
writeDocsAndStatusTests();
patchDemoIntegrationTest();
console.log("m34 patch v2 applied");

function patchPixiRenderer() {
  let text = read("src/render/pixiRenderer.ts");
  if (!text.includes("TerrainRenderSnapshot")) {
    text = replaceOnce(text, 'import type { ObstacleMaskRenderSnapshot } from "../sim/obstacleRenderSnapshot";', 'import type { ObstacleMaskRenderSnapshot } from "../sim/obstacleRenderSnapshot";\nimport type { TerrainRenderSnapshot } from "../sim/terrainRenderSnapshot";', "pixi terrain import");
  }
  if (!text.includes("terrainRenderSnapshot?: TerrainRenderSnapshot")) {
    text = replaceOnce(text, '  readonly obstacleMaskSnapshot: ObstacleMaskRenderSnapshot;\n  readonly snapshotStats: RenderSnapshotStats;', '  readonly obstacleMaskSnapshot: ObstacleMaskRenderSnapshot;\n  readonly terrainRenderSnapshot?: TerrainRenderSnapshot;\n  readonly snapshotStats: RenderSnapshotStats;', "frame terrain snapshot");
  }
  if (!text.includes("const terrainLayer = new Graphics();")) {
...
function patchDebugOverlay() {
  ...
  if (!text.includes("readonly terrainRenderMs")) {
  ...
  if (!text.includes('terrainRenderMs: createValueRow(root, "terrain render")')) {
  ...
  if (!text.includes("rows.terrainRenderMs.textContent")) {
...
function writeDocsAndStatusTests() {
  write("docs/integration_m34.md", [
    ...
    "",
    "Milestone 34 adds an optional Pixi terrain debug render layer that consumes the m33 terrain render snapshot boundary.",
    "",
    "## Implementation",
    "",
    "- `pixiRenderer.ts` accepts an optional `terrainRenderSnapshot` frame field.",
    "- `renderTerrainLayer()` draws material cells behind obstacles and agents.",
...
function replaceFileText(file, replacements) { let text = read(file); for (const [a,b] of replacements) text = text.replace(a,b); write(file,text); }
function read(file) { return readFileSync(join(root,file), "utf8"); }
function write(file,text) { mkdirSync(dirname(join(root,file)), { recursive: true }); writeFileSync(join(root,file), text, "utf8"); }
function replaceOnce(text, search, replacement, label) { if (!text.includes(search)) throw new Error("Missing anchor: " + label); return text.replace(search, replacement); }
